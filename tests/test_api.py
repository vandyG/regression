...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `lr`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in lr.__all__ or not hasattr(lr, obj.name)
    ]
